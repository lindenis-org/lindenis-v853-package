...
static int wpa_parse_signal_info(char *status, wifi_sta_info_t *sta_info)
{
	int ret = -1;
	char rssi[32] = {0};
	char *pch;
...
static void try_start_udhcpc()
{
	linux_private_data_t *private_data = NULL;
	private_data = (linux_private_data_t *)sta_wpa_inf_object.sta_private_data;
...
static int linux_supplicant_init(sta_wpa_event_cb_t sta_event_cb)
{
	if(sta_wpa_inf_object.sta_wpa_init_flag == WMG_FALSE) {
		...
		memset(((linux_private_data_t *)(sta_wpa_inf_object.sta_private_data))->old_ssid, 0,
				SSID_MAX_LEN);
		memset(((linux_private_data_t *)(sta_wpa_inf_object.sta_private_data))->new_ssid, 0,
				SSID_MAX_LEN);
...
static linux_private_data_t linux_private_data = {
	.old_ssid = {0},
	.new_ssid = {0},
	.last_time = 0,
};
