...
void *restart_ap_thread(void *arg)
{
	int now_time = time((time_t *)NULL);
	int last_time = now_time;
	int ret;

	while(restart_ap.restart_ap_flag) {
		now_time = time((time_t *)NULL);
		if((now_time - last_time) > RESTART_AP_TIME) {
			WMG_INFO("Time is up(%d:%d), restart ap now\n", (now_time - last_time), RESTART_AP_TIME);
			last_time = now_time;
			wifi_off();
			ret = wifi_on(WIFI_AP);
			if (ret == 0) {
				deamon_object.deamon_current_mode = WIFI_AP;
				WMG_INFO("wifi on restart ap mode success\n");
			} else {
				WMG_ERROR("wifi on restart ap mode failed\n");
				return;
			}
				WMG_INFO("restart ap enable success, ssid=%s, psk=%s, sec=%d, channel=%d\n",
						restart_ap.ap_config.ssid, restart_ap.ap_config.psk, restart_ap.ap_config.sec, restart_ap.ap_config.channel);
			ret = wifi_ap_enable(&(restart_ap.ap_config));
			if (ret == 0) {
				WMG_INFO("restart ap enable success, ssid=%s, psk=%s, sec=%d, channel=%d\n",
						restart_ap.ap_config.ssid, restart_ap.ap_config.psk, restart_ap.ap_config.sec, restart_ap.ap_config.channel);
			} else {
			...
			WMG_INFO("ap restart it's not time yet, keep sleeping\n");
			sleep(60);
		}
	};
	WMG_INFO("Exit restart ap thread\n");
	pthread_exit(0);
}
#endif
		...
		case WIFI_MSG_ID_MONITOR:
			switch(msg->data.mon_state) {
				...
				case WIFI_MONITOR_ENABLE:
					...
					break;
				default:
					WMG_INFO("***** wifi msg cb: monitor state unknow *****\n");
					break;
			}
			break;
		default:
...
#ifdef RESTART_AP
			restart_ap.restart_ap_flag = 0;
#endif

			WMG_INFO("wifi register msg cb\n");
			ret_cb = wifi_register_msg_cb(wifi_msg_cb);
			if(ret_cb) {
...
#ifdef RESTART_AP
			restart_ap.restart_ap_flag = 1;
			memcpy((void *)&restart_ap.ap_config, (void *)&ap_config, sizeof(wifi_ap_config_t));
			memcpy((void *)restart_ap.ssid_buf, (void *)ap_config.ssid, SSID_MAX_LEN);
			memcpy((void *)restart_ap.psk_buf, (void *)ap_config.psk, PSK_MAX_LEN);
			restart_ap.ap_config.ssid = restart_ap.ssid_buf;
			restart_ap.ap_config.psk = restart_ap.psk_buf;
			pthread_create(&(restart_ap.ap_restart_id), NULL, restart_ap_thread, NULL);
			WMG_INFO("restart ap enable success, ssid=%s, psk=%s, sec=%d, channel=%d\n",
				restart_ap.ap_config.ssid, restart_ap.ap_config.psk, restart_ap.ap_config.sec, restart_ap.ap_config.channel);
#endif
...
static int cmd_handle_s(char *p)
{
	...
	if (ret == 0) {
		...
		WMG_INFO("===Wi-Fi scan successful, total %d ap(buff size: %d) time %4f ms===\n", bss_num, RES_LEN, (((end - begin) / (double)sc_clk_tck) * 1000));
		return 0;
	} else {
...
static int cmd_handle_c(char *p)
{
	...
	if (ret == 0) {
		...
		if(cn_para.sec == WIFI_SEC_WPA2_PSK) {
			WMG_INFO("===Wi-Fi connect use wep===\n");
...
static int cmd_handle_p(char *p)
{
	...
	if(!strncmp(p, "ble", 3)) {
		...
		wifi_ap_config_t ap_config;
		char ssid_buf[SSID_MAX_LEN] = "Aw-config-net-Test";
