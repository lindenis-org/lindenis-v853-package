...
static void *_a2dp_source_thread_func(void *arg)
{
    ...
    snd_pcm_format_t native_format = SND_PCM_FORMAT_UNKNOWN;
...
start:
    ...
    BTMG_INFO("wav info, data size:%d, ch:%d, sample:%d, block:%d, bits_s:%d, path:%s", wav_header.data_size,
            wav_header.num_channels, wav_header.sample_rate, wav_header.block_align, wav_header.bits_per_sample, muisc_path);

    if (wav_header.riff_type == WAV_RIFF)
        big_endian = 0;
    else if (wav_header.riff_type == WAV_RIFX)
        big_endian = 1;
    else {
        close(fd);
        pthread_exit((void *)-1);
    }
    channels = TO_CPU_SHORT(wav_header.num_channels, big_endian);

    switch (TO_CPU_SHORT(wav_header.bits_per_sample, big_endian)) {
        case 8:
        ...
        case 16:
            ...
            break;
        case 24:
            switch (TO_CPU_SHORT(wav_header.block_align, big_endian) / channels) {
            case 3:
                if (big_endian)
            ...
            default:
                BTMG_ERROR("can't play WAVE-files with sample %d bits in %d bytes wide (%d channels)",
                    TO_CPU_SHORT(wav_header.bits_per_sample, big_endian), TO_CPU_SHORT(wav_header.block_align, big_endian), channels);
                close(fd);
                pthread_exit((void *)-1);
            }
            break;
        case 32:
            if (TO_CPU_SHORT(wav_header.compression_code, big_endian) == WAV_FMT_PCM) {
                if (big_endian)
            ...
            break;
        default:
            BTMG_ERROR("can't play WAVE-files with sample %d bits wide", TO_CPU_SHORT(wav_header.bits_per_sample, big_endian));
            close(fd);
            pthread_exit((void *)-1);
        }
        rate = TO_CPU_INT(wav_header.sample_rate, big_endian);

    if (bt_manager_a2dp_src_init(channels, rate, (int)native_format) != 0) {
