                      ...
                      int64_t       nCurrentTimeUs)
{
    ...
    dy_log("DecodeVideoStream, pDecoder=%p, \
            bEndOfStream=%d, bDropBFrameIfDelay=%d, nCurrentTimeUs=%ld", \
            pDecoder, bEndOfStream, bDropBFrameIfDelay, (long int)nCurrentTimeUs);
                          ...
                          int                  nStreamBufIndex)
{
    ...
    dy_log("Submit, pDecoder=%p, pDataInfo=%p, Index=%d, len = %d, pts = %ld",
            pDecoder, pDataInfo, nStreamBufIndex,
            pDataInfo->nLength, (long int)pDataInfo->nPts);
                                        ...
                                        VideoPicture* pVideoPicture, int bForbidUseFlag)
{
    ...
    if((CdcIonGetMemType() == MEMORY_IOMMU) && p->vconfig.bGpuBufValid)
    {
        ...
        logv("phyYBufAddr:%x", (unsigned int)pVideoPicture->phyYBufAddr);
//#ifdef __ANDROID__
