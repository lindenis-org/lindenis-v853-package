...
static int setLayerBuffer(LayerContext* lc,LayerBufMode mode)
{
    ...
    */
    if (strcmp(vd_platform, "r40") == 0) {//0x1701
        ...
        ve_offset = 0;
    } else if (strcmp(vd_platform, "d1") == 0) {//0x1859
        ve_offset = 0;
    } else {
...
int createG2dRotateBuf(LayerCtrl* l,VideoPicture* pBuf,int rotateDegree){
    ...
    //Since,open the lbc,G2D does not support direct rotation of 180бу,
    //the buffer is used to assist the rotation of 90бу twice.
    if ((rotateDegree == 180) && (pBuf->nLbcLossyComMod != 0))
    {
        ...
        lc->mRotateExtraBufInfo.pPicture.nLbcSize = pBuf->nLbcSize;
