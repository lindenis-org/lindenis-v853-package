...
static void shell_push_history(struct cli_shell *shell)
{
	if (shell->line_position != 0) {
		...
		if (shell->history_count >= CLI_HISTORY_LINES) {
			...
			memset(&shell->cmd_history[shell->history_count][0], 0,
			       CLI_CMD_SIZE);
			memcpy(&shell->cmd_history[shell->history_count][0],
		...
		while ((ch = getchar()) != 0) {
			...
			/* handle backspace key */
				...
				if (shell->line_position > shell->line_curpos) {
					...
					for (i = shell->line_curpos;
					     i <= shell->line_position; i++)
						printf("\b");
				} else {
					printf("\b \b");
					shell->line[shell->line_position] = 0;
