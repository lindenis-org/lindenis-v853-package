...
void healthd_mode_tina_battery_update(struct BatteryProperties *props)
{
	...
	int bat_temp_low, bat_temp_over, bat_vol_low, bat_cap_low;
	...
	DLOG("before tina ops update:ac-%u,usb-%u,bat-%u,bat_status-%u,"
		"bat_health-%u,bat_cap-%u,bat_vol-%u,bat_temp-%d\n",
	...
	if (props->batteryPresent &&
		...
		if (low_capacity_shutdown >= 0 &&
			...
			TLOGE("Battery level is lower than warning level2\n");
		...
		if (low_capacity_warning >= 0 &&
			...
			TLOGE("Battery level is lower than warning level1\n");
		}
#else
		/*low capacity handle */
		if (batinfo_get_bat_capacity_level() == 1 &&
				props->batteryLowCapWarn == false) {
			TLOGE("Battery level is lower than warning level1\n");
		}

		if (batinfo_get_bat_capacity_level() == 2) {
			TLOGE("Battery level is lower than warning level2\n");
			// perform_shutdown(props);
		}
		...
		if (low_capacity_warning >= 0 &&
			props->batteryLevel <= low_capacity_warning) {
			props->batteryLowCapWarn = true;
		} else if (low_capacity_warning >= 0 &&
			props->batteryLevel > low_capacity_warning) {
			props->batteryLowCapWarn = false;
		}
#endif
		/* low voltage handle */
		if (props->batteryVoltage > 0 &&
			get_healthd_config("bat_vol_low", &bat_vol_low) &&
			props->batteryVoltage <= bat_vol_low) {
			TLOGE("Battery Voltage:%d is lower than %duV\n",
				props->batteryVoltage, bat_vol_low);
			perform_shutdown(props);
		}

		/* low capacity handle */
		if (props->batteryLevel >= 0 &&
			get_healthd_config("bat_cap_low", &bat_cap_low) &&
			props->batteryLevel <= bat_cap_low) {
			TLOGE("Battery Capacity:%d is lower than %d\n",
				props->batteryLevel, bat_cap_low);
			perform_shutdown(props);
		}
	}
	...
	if (props->batteryPresent) {
		...
		if (props->batteryTemperature != -0xFF &&
			...
			props->batteryTemperature <= (bat_temp_low * 10)) {
			TLOGE("props->batteryTemperature:%d <= bat_temp_low:%d\n",
				props->batteryTemperature, (bat_temp_low * 10));
			perform_shutdown(props);
		}
		if (props->batteryTemperature != -0xFF &&
		    	get_healthd_config("bat_temp_over", &bat_temp_over) &&
			props->batteryTemperature >= (bat_temp_over * 10)) {
			TLOGE("props->batteryTemperature:%d >= bat_temp_over:%d\n",
				props->batteryTemperature, (bat_temp_over * 10));
			perform_shutdown(props);
		}
	}
	...
	if (ic_temp >= 0) {
		if (get_healthd_config("ic_high_temp_shutdown", &ic_high_temp_shutdown)
			...
			TLOGE("ic_temp:%d >= ic_high_temp_shutdown:%d, system shutdown\n",
					ic_temp, ic_high_temp_shutdown);
			perform_shutdown(props);
		}
