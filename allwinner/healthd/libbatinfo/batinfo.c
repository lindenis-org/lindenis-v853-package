...
{
	return (getIntField("/sys/class/power_supply/battery/capacity_alert_min") + 5);
}
...
int batinfo_get_bat_capacity_level()
{
	char buf[128];
	memset(buf, 0, sizeof(buf));

	if(readFromFile("/sys/class/power_supply/battery/capacity_level", buf, sizeof(buf)) > 0) {
		if (!strcmp(buf, "Low"))
			return 1;
