...
static void print_usage(const char *prog)
{
	...
	printf("speed test(trans) eg:uart_test -d 1 -n 100 -t 10 -f\n");
	printf("speed test(recive) eg:uart_test -d 1 -n 100 -t 10 -f -m\n");
...
static void parse_opts(int argc, char *argv[])
{
	while (1) {
		...
		c = getopt(argc, argv, "d:b:n:s:t:fgrhlm");
...
int main(int argc, char *argv[])
{
	...
	int i = 0, err_cnt = 0, suc_cnt = 0, io_cnt = 0, ret = 0, fd = 0, temp = 0, fd_loop;
